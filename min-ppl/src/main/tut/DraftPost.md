...
```
which are not strictly necessary, but clean up the subsequent code. We are going to use a set of weighted particles to represent a probability distribution empirically, so we'll start by defining an appropriate [ADT](https://en.wikipedia.org/wiki/Algebraic_data_type) for these:
```tut:silent
