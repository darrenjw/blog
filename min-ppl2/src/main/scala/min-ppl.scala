...
object MinPpl2 {
  ...
  trait Prob[T] {
    ...
    def map[S](f: T => S): Prob[S] = this.mapP(v => Particle(f(v), 0.0))
    ...
    def cond(ll: T => Double): Prob[T] = this.mapP(v => Particle(v, ll(v)))
  ...
  trait Dist[T] extends Prob[T] {
    ...
    def fit(obs: Seq[T]): Prob[T] = this.mapP(v => Particle(v, ll(obs)))
